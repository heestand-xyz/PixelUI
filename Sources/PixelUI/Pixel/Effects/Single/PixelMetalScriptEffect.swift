...
/// Pixel Metal Script Effect
///
/// **Variables:** pi, u, v, uv, w, h, wu, hv, tex, pix, red, green, blue, alpha, white, var.width, var.height, var.aspect, var.your-variable-name
public struct PixelMetalScriptEffect: Pixel {
    
    typealias Pix = MetalScriptEffectPIX
    
    public let pixType: PIXType = .effect(.single(.metalScriptEffect))
    ...
    public init(variables: [PixelVariable] = [],
        ...
        for key in Key.allCases {
            switch key {
            case .colorStyle:
                metadata[key.rawValue] = MetalScriptEffectPIX.ColorStyle.color.rawValue
            case .redScript:
    ...
    public init(variables: [PixelVariable] = [],
        ...
        for key in Key.allCases {
            switch key {
            case .colorStyle:
                metadata[key.rawValue] = MetalScriptEffectPIX.ColorStyle.white.rawValue
            case .whiteScript:
...
struct PixelMetalScriptEffect_Previews: PreviewProvider {
